    ...
    async def direct_call_tool(
        ...
        async with self:  # Ensure server is running
            try:
                ...
                # The MCP client session runs in an anyio task group, so a tool-call
                # error can surface wrapped in an `ExceptionGroup` rather than as a
                # bare `McpError`. Convert it to a recoverable `ModelRetry` (matching
